...
def draw_axis(img, yaw, pitch, roll, tdx=None, tdy=None, size = 80):
    ...
    #우선 이부분이 파란축이고 저희가 제일 필요할 것으로 생각되어집니다.
    x3 = size * (sin(yaw)) + tdx
    ...
    #해당함수가 Right, Center, Left 구분을 함
    convert_para(tdx,x3)

    return img

#Right, Center, Left 출력 관련된 함수
...
def main():
    ...
    for i in range(1, 1000):
        path = os.path.sep.join("Images", str(i))
        if os.path.exists(path):
            targetImageSet.append(cv2.imread(path, cv2.IMREAD_UNCHANGED))
        else:
            break
    
    for input_img in targetImageSet:
